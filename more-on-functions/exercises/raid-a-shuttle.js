...
//reduce the fuel level as much as possible WITHOUT changing the color returned by the checkFuel function.
...
let notStolenFuel = (nonSuspiciousFunction(fuelLevel));
console.log(fuelLevel);
console.log(notStolenFuel);

//Define another anonymous function with an array as a parameter, and set it equal to another innocent variable.
let nonSuspiciousFunctionTwo = function(arr) {
  arr.splice(0,2, 'sawdust', 'unitards');
return arr;
}
let notStolenCargo = (nonSuspiciousFunctionTwo(cargoHold));
console.log(cargoHold);
console.log(notStolenCargo);

//let varName = (cargoHold.splice(2, 2, “paper”, “sticks"));

let irs = function(levelOfFuel, itemsInCargo) {
  let arr = (nonSuspiciousFunctionTwo(cargoHold));
  return `Raided ${nonSuspiciousFunction(fuelLevel)} kg of fuel from the tanks, and stole ${arr[0]} and ${arr[1]} from the cargo hold.`
}
let notStolenFuelAndCargo = irs(notStolenFuel,notStolenCargo);
console.log(notStolenFuelAndCargo);
